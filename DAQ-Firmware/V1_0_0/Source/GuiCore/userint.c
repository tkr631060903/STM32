...
int ProcessSystemEvents ()
{
    ...
    u16 ph;
     ...
     ////PEN_STATUS: 触摸状态,16点,4左,8右,1上,2下,64 长按
    if (PEN_STATUS & 0x20)//按下
    {
        PEN_STATUS = 0;
        ...
        if(keyDownControlId)
        {
            //2015-10-31 暂不支持按下之后的效果以提速
            //默认按下效果
            if(GuiCtrl[keyDownControlId].ControlId!= NULL && (GuiCtrl[keyDownControlId].ItemType ==TYPE_COMMANDBUTTON || GuiCtrl[keyDownControlId].ItemType ==TYPE_STRING) )//控件才支持按下之后变效果
            {
                if(UpperPanelId)
