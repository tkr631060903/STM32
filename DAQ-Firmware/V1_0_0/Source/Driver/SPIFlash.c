...
void sFLASH_LowLevel_Init(void)
{
  ...
  GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable,ENABLE) ;//针对SPI3跟JTAG复用的问题，关JTAG，开SWD。
}
...
void SPI1_DMA_Configuration(u32 len)
{
    ...
    DMA_InitTypeDef DMA_InitStructure;      //DMA设置结构体
    ...
    DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_HalfWord;//DMA_PeripheralDataSize_Word;//DMA_PeripheralDataSize_HalfWord
    DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_HalfWord;//DMA_MemoryDataSize_Word;//DMA_MemoryDataSize_HalfWord
    ...
    //DMA_ITConfig(DMA1_Channel2, DMA_IT_TC, DISABLE); //设置传输完成后中断
    SPI_I2S_DMACmd(SPI1, SPI_I2S_DMAReq_Rx, ENABLE);      //* Enable SPI1 /      
    
    
}
void StartSPIDMATransfer()
{
    DMA_Cmd(DMA1_Channel2, ENABLE);//* 打开DMA通道 /
...
void sFLASH_Init(void)
{
  ...
  //SPI_SSOutputCmd(sFLASH_SPI, ENABLE);
  SPI_I2S_DMACmd(sFLASH_SPI, SPI_I2S_DMAReq_Rx, ENABLE);
...
void sFLASH_ReadUniqueID(char* str)
{
  ...
  /*!< Send "RDID " instruction */
  sFLASH_SendByte(0x4B);

  for(i=0;i<4;i++)
    sFLASH_SendByte(sFLASH_DUMMY_BYTE);
  
  for(i=0;i<8;i++)
      tmp[i] = sFLASH_SendByte(sFLASH_DUMMY_BYTE);

  /*!< Deselect the FLASH: Chip Select high */
  sFLASH_CS_HIGH();

  sprintf(str,"%02x%02x%02x%02x%02x%02x%02x%02x",tmp[0],tmp[1],tmp[2],tmp[3],tmp[4],tmp[5],tmp[6],tmp[7]);
