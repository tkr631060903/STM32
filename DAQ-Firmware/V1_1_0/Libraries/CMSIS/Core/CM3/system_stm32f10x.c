...
//#define SYSCLK_FREQ_HSE    HSE_Value 			//12MHz
...
/**
  ...
  */
  void SystemInit (void)
  {
      ...
      /* Set HSION bit */
      RCC->CR |= (uint32_t)0x00000001;//开启内部时钟HSI
      
      /* Reset SW, HPRE, PPRE1, PPRE2, ADCPRE and MCO bits */
#ifndef STM32F10X_CL
      RCC->CFGR &= (uint32_t)0xF8FF0000;//选择内部时钟，复位SW, HPRE, PPRE1, PPRE2, ADCPRE and MCO
#else
      RCC->CFGR &= (uint32_t)0xF0FF0000;
#endif /* STM32F10X_CL */   
      
      /* Reset HSEON, CSSON and PLLON bits */
      RCC->CR &= (uint32_t)0xFEF6FFFF;//关闭时钟监测器，关闭HSE
...
static void SetSysClockTo72(void)
{
    ...
    if (HSEStatus == (uint32_t)0x01)//外部晶振成功启动,开始配置其它时钟系统
    {
        /* Enable Prefetch Buffer */
        FLASH->ACR |= FLASH_ACR_PRFTBE;
        ...
        RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_PLLSRC | RCC_CFGR_PLLXTPRE |
                                            RCC_CFGR_PLLMULL));
        //RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_HSE | RCC_CFGR_PLLMULL6);//8MHz
        //外部晶振16MHz/2*9 = 72MHz
