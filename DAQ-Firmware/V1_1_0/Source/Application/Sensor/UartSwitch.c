...
void USART1_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(USART1, USART_IT_IDLE) != RESET) 
    {
        ...
        if(gUART3Bond ==1)
        {
            if(bufLen=GetUart1RxStatus(strbuf))
            {
                SendData(3,strbuf,bufLen);
            }
        }
    }
}
...
void USART2_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(USART2, USART_IT_IDLE) != RESET) 
    {
        ...
        if(gUART3Bond ==2)
        {
            if(bufLen=GetUart2RxStatus(strbuf))
            {
                SendData(3,strbuf,bufLen);
            }
        }
...
void USART3_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)          //判断读寄存器是否非空
    {
        ...
        if(Uart3_Rx_Buffer[Uart3_Rx_Index]==0x0a)
        {
          if(gUart3_Hard_Reset_Enable && Uart3_Rx_Buffer[Uart3_Rx_Index+UART3_BUFFER_SIZE-15]== '{' && Uart3_Rx_Buffer[Uart3_Rx_Index+UART3_BUFFER_SIZE-14]== '"' &&  Uart3_Rx_Buffer[Uart3_Rx_Index+UART3_BUFFER_SIZE-13]== 'c' &&  Uart3_Rx_Buffer[Uart3_Rx_Index+UART3_BUFFER_SIZE-12]== 'm' &&  Uart3_Rx_Buffer[Uart3_Rx_Index+UART3_BUFFER_SIZE-11]== 'd' &&  Uart3_Rx_Buffer[Uart3_Rx_Index+UART3_BUFFER_SIZE-10]== '"' 
...
void UART4_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(UART4, USART_IT_IDLE) != RESET) 
    {
        ...
        if(gUART3Bond ==4)
        {
            if(bufLen=GetUart4RxStatus(strbuf))
            {
                SendData(3,strbuf,bufLen);
            }
        }
...
void UART5_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(UART5, USART_IT_IDLE) != RESET) 
    {
        ...
        if(gUART3Bond ==5)
        {
            if(bufLen=GetUart5RxStatus(strbuf))
            {
                SendData(3,strbuf,bufLen);
            }
        }
