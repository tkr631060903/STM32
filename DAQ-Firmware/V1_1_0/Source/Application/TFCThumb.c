...
const  char DAQSoftwareVersionInfo[]= {'D','A','Q','-',BUILD_YEAR_CH0, BUILD_YEAR_CH1, BUILD_YEAR_CH2, BUILD_YEAR_CH3, BUILD_MONTH_CH0, BUILD_MONTH_CH1, BUILD_DAY_CH0, BUILD_DAY_CH1, '-', BUILD_HOUR_CH0,BUILD_HOUR_CH1,BUILD_MIN_CH0,BUILD_MIN_CH1,BUILD_SEC_CH0,BUILD_SEC_CH1,'\0'};
const  char PWRSoftwareVersionInfo[]= {'P','W','R','-',BUILD_YEAR_CH0, BUILD_YEAR_CH1, BUILD_YEAR_CH2, BUILD_YEAR_CH3, BUILD_MONTH_CH0, BUILD_MONTH_CH1, BUILD_DAY_CH0, BUILD_DAY_CH1, '-', BUILD_HOUR_CH0,BUILD_HOUR_CH1,BUILD_MIN_CH0,BUILD_MIN_CH1,BUILD_SEC_CH0,BUILD_SEC_CH1,'\0'}; 
...
int TFCCallbackSwVersion(cJSON* root, cJSON* retObj)
{
    ...
    if(hwVersion&0x08)
        cJSON_AddStringToObject(retObj, "sw_version", DAQSoftwareVersionInfo);
    else 
        cJSON_AddStringToObject(retObj, "sw_version", PWRSoftwareVersionInfo);
...
int TFCCallbackTFCConfig(cJSON* root, cJSON* retObj)
{
    ...
    if(sys.i2c_baud_rate_default>1000000)  sys.i2c_baud_rate_default = 1000000;
...
int TFCCallbackTfcInfo(cJSON* root, cJSON* retObj)
{
    ...
    u8 hwVersion= GetDeviceHardwareId(buf);
    ...
    if(hwVersion&0x08)//DAQ Mode
        cJSON_AddStringToObject(retObj, "tfc_hardware", "Liu Wenhao");
    else
        cJSON_AddStringToObject(retObj, "tfc_hardware", "Yang Rongde");
    ...
    {
        cJSON_AddStringToObject(retObj, "tfc_designer_email", "kim-yang@minieye.cc");
        cJSON_AddStringToObject(retObj, "tfc_designer_contact", "13825233540");
    }
...
/*  {29,0x00,0x00,TFCCallbackUartBond,"uart_bond"},
  {30,0x08,0x00,TFCCallbackPwrRegWrite,"pwr_reg_write"},
  {31,0x08,0x00,TFCCallbackPwrRegRead,"pwr_reg_read"},
  {32,0x08,0x00,TFCCallbackPwrWrite,"pwr_write"},
  {33,0x08,0x00,TFCCallbackPwrRead,"pwr_read"},*/
...
int TFCCallbackUartBond(cJSON* root, cJSON* retObj)
{
    ...
    if(cJSON_GetObjectItem(root,"operation")) operation = cJSON_GetObjectItem(root,"operation")->valuestring;
    if(strcmp(operation,"bond")==0)
    {
        if(cJSON_GetObjectItem(root,"uart_id")) uart_id = cJSON_GetObjectItem(root,"uart_id")->valueint;
        if(uart_id<1 || uart_id==3 || uart_id>5)
        {
            cJSON_AddStringToObject(retObj, "report", "uart_id must be 1/2/4/5" );
            return 0;
        }
        gUART3Bond = uart_id;
    }
    else if(strcmp(operation,"unbond")==0)
    {
        gUART3Bond = 0;
    }
    else
    {
        cJSON_AddStringToObject(retObj, "report", "operation must be 'bond' or 'unbond'." );
        return 0;
    }
    return 1;
}
...
{
    u8 writeData[255];
    static u8 devAddr=0x4F;
    static u8 length = 0;
    cJSON* cmdData;
    int cmdData_size;
    int i;
    
    if(cJSON_GetObjectItem(root,"devAddr")) devAddr = cJSON_GetObjectItem(root,"devAddr")->valueint;
    if(cJSON_GetObjectItem(root,"length")) length = cJSON_GetObjectItem(root,"length")->valueint;
    
    if(length)
    {
        if(!cJSON_GetObjectItem(root,"cmdData"))
        {
            cJSON_AddStringToObject(retObj, "report", "cmdData must not be empty when lenght is not zero." );
            return 0;
        }
        cmdData= cJSON_GetObjectItem(root,"cmdData");
        cmdData_size = cJSON_GetArraySize(cmdData);
        if(cmdData_size!=length)
        {
            cJSON_AddStringToObject(retObj, "report", "cmdData size must = length." );
            return 0;
        }
        for(i=0;i<cmdData_size;i++)
        {
            writeData[i]=cJSON_GetArrayItem(cmdData, i)->valueint;
        }
    }
    //I2C1_BufferWrite(u8 devAddr, u8* buffer, u8 WriteAddr, int numToWrite)
    if(!I2C1_BufferWrite(devAddr, writeData+1,writeData[0],length-1))
    {
        cJSON_AddStringToObject(retObj, "report", "I2C access failed." );
        return 0;
    }
    else return 1;
}
...
int TFCCallbackPwrRegRead(cJSON* root, cJSON* retObj)
{
    ...
    cJSON* cmdData;
    int cmdData_size=0;
    static int byte_to_read = 1;
    int i;
    
    if(cJSON_GetObjectItem(root,"devAddr")) devAddr = cJSON_GetObjectItem(root,"devAddr")->valueint;
    if(cJSON_GetObjectItem(root,"length")) length = cJSON_GetObjectItem(root,"length")->valueint;
    if(cJSON_GetObjectItem(root,"byte_to_read")) byte_to_read = cJSON_GetObjectItem(root,"byte_to_read")->valueint;
    if(length)
    {
        if(!cJSON_GetObjectItem(root,"cmdData"))
        {
    ...
    //void I2C1_BufferRead(u8 devAddr, u8* writeBuffer, u8 ReadAddr, int numToWrite, int NumByteToRead, u8* readBuffer)
    if(!I2C1_BufferRead(devAddr,writeData+1,writeData[0],length-1,byte_to_read,writeData))
    {
        cJSON_AddStringToObject(retObj, "report", "I2C access failed." );
        return 0;
    }
    ...
    {
        sprintf(tmpStr,"0x%x,",writeData[i]);
        strcat(ss,tmpStr);
    }
    cJSON_AddStringToObject(retObj, "read_back_data", ss);
    return 1;
}
/*
typedef struct
...
int TFCCallbackPwrWrite(cJSON* root, cJSON* retObj)
{
    ...
    channel = cJSON_GetObjectItem(root,"channel")->valueint;
    if(channel ) channel = 1;
    if(cJSON_GetObjectItem(root,"enable")) 
    {
        gPS.ch_state[channel] = cJSON_GetObjectItem(root,"enable")->valueint;
        if(gPS.ch_state[channel]) gPS.ch_state[channel]=1;
    }
    if(cJSON_GetObjectItem(root,"vset")) 
    {
        if(cJSON_GetObjectItem(root,"vset")->valueint>45000 || cJSON_GetObjectItem(root,"vset")->valueint<0)
        {
            cJSON_AddStringToObject(retObj, "report", "Vset should be in the range 0-45000." );
            return 0;
        }
        gPS.ch_vset[channel] = (float)(cJSON_GetObjectItem(root,"vset")->valueint)/1000;
    }
    if(cJSON_GetObjectItem(root,"iset")) 
    {
        if(cJSON_GetObjectItem(root,"iset")->valueint>5000 || cJSON_GetObjectItem(root,"iset")->valueint<0)
        {
            cJSON_AddStringToObject(retObj, "report", "Iset should be in the range 0-5000." );
            return 0;
        }
        gPS.ch_iset[channel] = (float)(cJSON_GetObjectItem(root,"iset")->valueint)/1000;
    }
    
    if(UpdatePowerParam())
    {
        cJSON_AddNumberToObject(retObj, "channel", channel );
        cJSON_AddNumberToObject(retObj, "enable", gPS.ch_state[channel] );
        cJSON_AddNumberToObject(retObj, "vset", (gPS.ch_vset[channel]+0.0005)*1000 );
        cJSON_AddNumberToObject(retObj, "iset", (gPS.ch_iset[channel]+0.0005)*1000 );
        cJSON_AddNumberToObject(retObj, "vrt", (gPS.ch_vrt[channel]+0.0005)*1000 );
        cJSON_AddNumberToObject(retObj, "irt", (gPS.ch_irt[channel]+0.0005)*1000 );
        return 1;
    }
    else
    {
        cJSON_AddStringToObject(retObj, "report", "Update Param failed." );
        return 0;
    }
}
