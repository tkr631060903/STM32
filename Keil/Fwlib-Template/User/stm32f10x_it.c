...
{
  if(EXTI_GetITStatus(EXTI_Line0) != RESET) //检测外部中断状态，外部接高电平判断置位，外部接地则判断复位
  {
    //TOGGLE();
    Light();
...
{
  if(EXTI_GetITStatus(EXTI_Line13) != RESET)  //检测外部中断状态，外部接高电平判断置位，外部接地则判断复位
  {
